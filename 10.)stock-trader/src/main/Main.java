...
public class Main {
    ...
    public static void main(String[] args) {    
        ...
        switch (accountChoice()) {
            ...
            case 'b': account = new TFSA(INITIAL_DEPOSIT);
        }
        initialBalance();
        ...
        for (int day = 1; day <= 2160; day++) {
            ...
            displayPrices(day);
            String choiceBuyOrSell = buyOrSell();
            Type buyOrSell = choiceBuyOrSell.equals("buy") ? Type.MARKET_BUY : Type.MARKET_SELL;

            String choiceStock = chooseStock();
            Stock stock = Stock.valueOf(choiceStock);

            int numOfShares = numShares(choiceBuyOrSell);
            double price = Double.parseDouble(getPrice(stock, day));

            Trade trade = new Trade(stock, buyOrSell, numOfShares, price);
            String result = account.successOrNot(trade);
            
            account.executeTrade(trade);
            tradeStatus(result);
        }
    ...
    public static char accountChoice() {
